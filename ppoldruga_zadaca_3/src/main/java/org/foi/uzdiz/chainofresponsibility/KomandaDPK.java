	...
	protected void obradi(String komanda) {
		...
		if (dijelovi.length < 2) {
			UpraviteljGresaka.getInstance()
					.greskaURadu("Greška: Nedostaju parametri. Sintaksa: DPK ime prezime - oznakaVlaka [- stanica]");
			return;
		}
		...
		if (korisnikPodaci.length < 2) {
			UpraviteljGresaka.getInstance().greskaURadu(
					"Greška: Neispravno ime i prezime. Sintaksa: DPK ime prezime - oznakaVlaka [- stanica]");
			return;
		}
		...
		if (korisnik == null) {
			UpraviteljGresaka.getInstance()
					.greskaURadu("Korisnik " + ime + " " + prezime + " nije pronađen u registru.");
			return;
		}
		...
		korisnik.dodajPracenjeVlaka(oznakaVlaka);
		if (stanica != null) {
			korisnik.dodajPracenjeStanice(stanica);
		}
		...
		System.out.println("Korisnik " + korisnik
				+ (stanica != null ? " prati stanicu " + stanica : " prati vlak " + oznakaVlaka) + ".");
